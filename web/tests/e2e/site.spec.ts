...
test("home runs the example scan under a subpath and shows findings", async ({ page }) => {
  ...
  await expect(page.getByText("Example · openai-python-model-config")).toBeVisible();
  await expect(page.getByText("gpt-4-turbo").first()).toBeVisible();
  await expect(page.getByText("Breaking · act before a date")).toBeVisible();
  // callsite layer ran: the inventory lists the SDK call found through tree-sitter
  await expect(page.getByText("chat.completions.create").first()).toBeVisible();
...
test("a different sample can be scanned from the picker", async ({ page }) => {
  ...
  await page.locator("#sample-select").selectOption("stripe-java-sources");
