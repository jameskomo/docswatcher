...
<template>
  <div class="stack">
      ...
      <table>
        ...
        <tbody>
          <tr v-for="c in rows" :key="c.id">
            ...
            <td class="muted">{{ KIND_LABEL[c.kind] ?? c.kind }}</td>
            <td class="mono">
              <span class="trunc" style="display: inline-block; max-width: 34ch; vertical-align: bottom" :title="c.key">{{ c.key }}</span>
              <span v-if="c.context?.sdk?.version" class="muted"> · {{ c.context.sdk.version }}</span>
            </td>
            ...
            <td class="mono" :title="allLocations(c)">
              ...
              <span v-if="c.evidence.length > 1" class="muted"> +{{ c.evidence.length - 1 }}</span>
            </td>
            <td>
    ...
    <div v-else class="empty">
      ...
      <p class="small" v-if="filter">Clear the filter to see all {{ contracts.length }} contracts.</p>
      <p class="small" v-else-if="lowCount">Only low-confidence mentions were found. Tick the box above to see them.</p>
      <p class="small" v-else>This scan found no external API contracts in the files it read.</p>
    </div>
  </div>
</template>
