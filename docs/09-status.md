...
| Server app | Webhooks, scan worker, REST API, fix dispatch, runtime observation with per-repository ingest tokens, org blast radius, sign-in with GitHub with per-organisation and per-repository access (ADR 0008) | 168 tests, run in CI (they need Postgres); the OAuth round trip runs against a scripted GitHub; fix pull requests run an agent that can only edit the files a finding names |
