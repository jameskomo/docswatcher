...
- The Content-Security-Policy `connect-src` (`deployment/security-headers.conf`) names the three
  API hosts and nothing new besides.
...
- Any script running on this origin could read a key typed into the page. The page renders no
  user-supplied HTML, and its policy allows scripts only from itself, Nuxt's inline bootstrap, and
  Cloudflare's analytics beacon, which the edge injects. Adding any other third-party script to
  the site would put visitors' keys in its reach; don't.
