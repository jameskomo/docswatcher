...
### When the date passes: `active` becomes `expired`

The status has to agree with the calendar, and `docswatcher validate` checks it against today:

| Status | Effective date | Validate says |
|---|---|---|
...
- [ ] `effective` is after `announced`, and an `active` record has a future or null `effective`. If validate warns that it takes effect within 7 days, expect the expiry pull request the day after.
